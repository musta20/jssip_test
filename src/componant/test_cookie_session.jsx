...
class  Mainnav extends Component {
    constructor(props){
      ...
      this.state = { 
        ...
        chat:[],
        SIPuser:null }
        this.remoteView = React.createRef()
      ...
      //  const mediaSource = new MediaSource();
        ...
        this.state.session.connection.addEventListener('addstream',function(e) {
         ...
         this.remoteView.current.srcObject = (e.stream);
    ...
    render() { 
        ...
        return (
        <div><h5>WebRT</h5>
                  <video className='video' ref={this.remoteView} autoPlay muted={true}></video>
            <h3><video className='video' src="https://www.youtube.com/watch?v=VeC6P-Dwl6Q" ref={this.youtubeView} autoPlay muted={true}></video>
</h3>
            <h1>
           <br></br>
  ...
  const chat = [...state.chat, {mes:e.request.body,id:state.chat+1}];   
   ...
   this.state.SIPuser.on('newRTCSession', function(e){ 
    ...
    this.state.session = e.session;
     ...
     this.state.session.on('connecting', function() {console.log('CONNECT'); });
     this.state.session.on('peerconnection', this.add_stream ,console.log('peerconnection'));

     this.state.session.on('ended', e=>{this.setState({session:null}); console.log('ended')});
     this.state.session.on('failed', e=>{this.setState({session:null}); console.log('failed')});
